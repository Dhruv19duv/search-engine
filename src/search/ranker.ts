...
import { Document, Posting, SearchResult, RankingFeatures } from '../types';
...
export class Ranker {
  ...
  private readonly k3: number = 8.0;
  ...
  private domainAuthorityCache: Map<string, number>;
  ...
  /** Compute BM25 score */
  private computeBM25(queryTerms: string[], docId: number, docLength: number): number {
  ...
  private computeTFIDF(queryTerms: string[], docId: number, docLength: number): number {
    const N = this.index.documentCount();
    let score = 0;
  ...
  private computeScore(features: RankingFeatures, queryTerms: string[], rawQuery: string): number {
