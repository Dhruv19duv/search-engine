...
class SnippetGenerator {
    maxSnippetLength;
    minSnippetLength;
    lambda; // MMR diversity parameter (0 = max relevance, 1 = max diversity)
    constructor(maxLength = 300, minLength = 100, lambda = 0.7) {
        this.maxSnippetLength = maxLength;
        this.minSnippetLength = minLength;
        this.lambda = lambda;
    }
    /** Generate a snippet for a document given a query */
    generateSnippet(content, query, title) {
    ...
    mmrScoring(sentences, queryTerms) {
        const selected = [];
        const scored = [];
    ...
    sentenceQuerySimilarity(sentence, queryTerms) {
    ...
    buildSnippet(scored, queryTerms, title) {
        if (scored.length === 0)
            return '';
        ...
        for (const s of scored) {
            ...
            if (snippet.length >= this.minSnippetLength)
                break;
        }
    ...
    generateMetadataSnippet(doc, query) {
