...
export class SimHash {
  ...
  constructor(fingerprintSize: number = 64, threshold: number = 3) {
  ...
  compute(content: string): bigint {
    ...
    if (tokens.length < 3) return BigInt(0);
   ...
   */
  private hashString(data: string): bigint {
  ...
  private shingleWords(tokens: string[], n: number): string[] {
   ...
   */
  private computeShingleWeights(shingles: string[]): Map<string, number> {
   ...
   */
  lshBuckets(fingerprint: bigint, bands: number = 4): string[] {
    const rowsPerBand = Math.ceil(this.fingerprintSize / bands);
    const buckets = new Array<string>(bands);
   ...
   */
  findNearDuplicates(
    fingerprints: Map<number, bigint>,
    bands: number = 16
  ): Map<number, number[]> {
    if (fingerprints.size < 2) return new Map();
      ...
      for (let i = 0; i < ids.length; i++) {
        for (let j = i + 1; j < ids.length; j++) {
          ...
          if (this.hammingDistance(fpA, fpB) <= this.threshold) {
            if (!duplicates.has(a)) duplicates.set(a, new Set());
            if (!duplicates.has(b)) duplicates.set(b, new Set());
