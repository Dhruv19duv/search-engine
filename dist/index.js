...
const query_processor_1 = require("./search/query-processor");
const ranker_1 = require("./search/ranker");
const spell_corrector_1 = require("./search/spell-corrector");
const snippet_generator_1 = require("./search/snippet-generator");
const query_understanding_1 = require("./ml/query-understanding");
const learning_to_rank_1 = require("./ml/learning-to-rank");
const dense_retrieval_1 = require("./ml/dense-retrieval");
const semantic_drift_1 = require("./ml/semantic-drift");
const spam_filter_1 = require("./ml/spam-filter");
const query_server_1 = require("./server/query-server");
class SearchEngine {
    // Core components
    crawler;
    indexer;
    ...
    constructor() {
        ...
        this.indexer = new indexer_1.Indexer(1000, 1000000);
        this.queryProcessor = new query_processor_1.QueryProcessor();
        this.ranker = new ranker_1.Ranker(this.indexer.getMainIndex());
        this.spellCorrector = new spell_corrector_1.SpellCorrector();
        this.snippetGenerator = new snippet_generator_1.SnippetGenerator();
        this.crawler = new crawler_1.Crawler({
            maxPages: 1000000,
            maxDepth: 10,
        ...
        this.queryUnderstanding = new query_understanding_1.QueryUnderstanding();
        this.learningToRank = new learning_to_rank_1.LearningToRank(50, 0.1, 6);
        this.denseRetrieval = new dense_retrieval_1.DenseRetrieval(256);
        this.semanticDriftDetector = new semantic_drift_1.SemanticDriftDetector(30, 64, 0.35);
        this.spamFilter = new spam_filter_1.SpamFilter(50, 0.5);
        this.server = null;
    ...
    async search(query, page = 1, pageSize = 10) {
        ...
        // Process query
        const parsedQuery = this.queryProcessor.processQuery(query);
        // Classify intent
        ...
        if (queryTerms.length > 0) {
            ...
            if (hasDrift) {
                for (const result of results) {
                    ...
                    if (doc) {
                        const driftScore = this.semanticDriftDetector.computeDocumentDriftScore(doc.title + ' ' + doc.content);
        ...
        for (const result of results) {
            const doc = docs.get(result.docId);
            if (doc) {
                result.snippet = this.snippetGenerator.generateSnippet(doc.content, effectiveQuery, doc.title);
            }
        }
        // Apply spam filter
        results = results.filter(result => {
            ...
            if (!doc)
                return false;
            const spamResult = this.spamFilter.classify(doc.content, doc.title, doc.url, doc.domain, doc.outgoingLinks, doc.headers);
    ...
    async reindex() {
        ...
        this.denseRetrieval = new dense_retrieval_1.DenseRetrieval();
        this.semanticDriftDetector = new semantic_drift_1.SemanticDriftDetector();
        this.ranker = new ranker_1.Ranker(this.indexer.getMainIndex());
    ...
    startServer(port = 3000) {
        this.server = new query_server_1.QueryServer(this, { port });
        this.server.start();
    }
    /** Stop the server */
    stopServer() {
