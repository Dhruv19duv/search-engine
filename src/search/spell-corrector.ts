...
export class SpellCorrector {
  ...
  private unigramCounts: Map<string, number>;
  private totalWords: number;
  private soundexCache: Map<string, string>;
  ...
  addToDictionary(word: string, frequency: number = 1): void {
    this.dictionary.add(word.toLowerCase());
    this.unigramCounts.set(word.toLowerCase(), 
      (this.unigramCounts.get(word.toLowerCase()) || 0) + frequency);
  ...
  private scoreCandidate(candidate: string, original: string, context: string[]): number {
    let score = 0;
  ...
  private contextScore(word: string, context: string[]): number {
    let score = 0;
  ...
  private isCommonBigram(w1: string, w2: string): boolean {
    const commonPairs = [
  ...
  private characterNGramSimilarity(a: string, b: string, n: number = 2): number {
  ...
  private getCharacterNGrams(word: string, n: number): Set<string> {
  ...
  private getSoundex(word: string): string {
    ...
    // Pad with zeros
    while (encoded.length < 4) encoded += '0';
  ...
  private soundexCode(char: string): string {
  ...
  /** Add common misspellings corrections */
  private addCommonCorrections(word: string, candidates: Set<string>): void {
  ...
  correctText(text: string): { corrected: string; corrections: Array<{ original: string; corrected: string }> } {
