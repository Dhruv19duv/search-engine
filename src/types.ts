...
export interface Document {
  ...
  language: string;
  isRemoved: boolean; // GDPR support
}
...
export interface ParsedQuery {
  ...
  type: QueryType;
  intent: SearchIntent;
  booleanOperators?: { term: string; operator: 'AND' | 'OR' | 'NOT' }[];
  phraseTerms?: string[];
  fuzzyTerms?: { term: string; maxDistance: number }[];
  correctedTerms?: { original: string; corrected: string; distance: number }[];
}
...
export interface RankingFeatures {
  ...
  driftScore: number; // semantic drift score
}
